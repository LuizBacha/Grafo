...
Ponto de entrada da aplicação. Apresenta o menu de opções do MOBILISP,
incluindo, na Parte 3, a solução do problema por caminho mínimo (Dijkstra)
e as técnicas de análise de características do grafo (graus, euleriano,
...
def menu():
    ...
    while True:
        ...
        print("-" * 55)
        print("  >>> Solução e Características <<<")
        print("12) Caminho mínimo entre dois pontos (Dijkstra)")
        print("13) Grau dos vértices")
        print("14) Verificar grafo/percurso euleriano")
        print("15) Verificar grafo/ciclo hamiltoniano")
        print("16) Coloração do grafo (nº de partições)")
        print("0) Encerrar")
        ...
        try:
            if opcao == "1":
                ...
                origem = int(input("Ponto de origem (ID): "))
                destino = int(input("Ponto de destino (ID): "))
                exibir_caminho_minimo(grafo, origem, destino)
            elif opcao == "13":
                exibir_graus(grafo)
            elif opcao == "14":
                exibir_euleriano(grafo)
            elif opcao == "15":
                exibir_hamiltoniano(grafo)
            elif opcao == "16":
                exibir_coloracao(grafo)
            elif opcao == "0":
